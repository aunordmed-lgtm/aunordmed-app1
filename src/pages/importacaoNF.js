...
// Parser XML NFS-e padrão nacional (ABRASF v2.04)
function parseXMLNFSe(xmlText) {
  ...
  // Tentar diferentes tags do padrão nacional
  const tags = ['CompNfse', 'Nfse', 'InfNfse']
  let nfsElements = []

  for (const tag of tags) {
    const els = doc.getElementsByTagName(tag)
    if (els.length > 0) { nfsElements = Array.from(els); break }
  }

  if (nfsElements.length === 0) {
    // Tentar pegar direto os campos
    nfsElements = [doc.documentElement]
  }
  ...
  nfsElements.forEach((el, i) => {
    try {
      const get = (tag) => {
        ...
        return found.length > 0 ? found[0].textContent?.trim() : null
      }

      const numero = get('Numero') || get('NumeroNfse') || get('NroNota') || `IMPORT-${i+1}`
      const valorBruto = parseFloat(get('ValorServicos') || get('ValorLiquidoNfse') || get('Valor') || '0')
      const tomador = get('RazaoSocial') || get('NomeFantasia') || get('Tomador') || 'Tomador não identificado'
      const cnpjTomador = get('Cnpj') || get('CpfCnpj') || ''
      const competencia = get('Competencia') || get('DataEmissao') || ''
      const dataEmissao = get('DataEmissao') || get('DhEmi') || ''
      const discriminacao = get('Discriminacao') || get('Descricao') || ''
      const iss = parseFloat(get('ValorIss') || get('ValorISSQN') || '0')
      const aliquotaIss = parseFloat(get('Aliquota') || '0')

      // Converter competência para formato YYYY-MM
      let comp = ''
      if (competencia) {
        const d = new Date(competencia)
        if (!isNaN(d)) comp = d.toISOString().substring(0, 7)
      }
      if (!comp && dataEmissao) {
      ...
      if (dataEmissao) {
        const d = new Date(dataEmissao)
        if (!isNaN(d)) emissao = d.toISOString().split('T')[0]
      }
    ...
    reader.onload = (e) => {
      try {
        ...
        rows.slice(1).forEach((row, i) => {
          if (!row.some(c => c !== '')) return // linha vazia
          ...
          try {
            ...
            if (emissaoRaw instanceof Date) emissao = emissaoRaw.toISOString().split('T')[0]
              ...
              const d = new Date(emissaoRaw)
              if (!isNaN(d)) emissao = d.toISOString().split('T')[0]
            }

            notas.push({
              nf: get(cols.nf) || `IMPORT-${i+1}`,
              tomador: get(cols.tomador) || 'Não informado',
              comp: comp || mesAtual(),
              emissao,
              bruto: valorBruto,
              status: get(cols.status) || 'Emitida',
              origem: 'excel'
            })
          } catch (e) {
        ...
        resolve({ notas: [], erros: ['Erro ao ler arquivo: ' + e.message] })
      }
    }
...
export function ImportacaoNF({ medicos, onRefresh }) {
  ...
  const processarArquivo = async (file) => {
    ...
    try {
      ...
      if (file.name.endsWith('.xml')) {
        ...
        resultado = parseXMLNFSe(text)
      } else {
        resultado = await parseExcel(file)
      }

      if (resultado.erros.length > 0) setErros(resultado.erros)

      if (resultado.notas.length > 0) {
        setNotasImportadas(resultado.notas)
        setSelecionadas(new Set(resultado.notas.map((_, i) => i)))
        setEtapa('preview')
        toast(`${resultado.notas.length} nota(s) identificada(s)!`)
      } else {
  ...
  const importar = async () => {
    ...
    for (const n of notasSel) {
      try {
        ...
        const medicos_nota = medicoSelecionado ? [{
          nome: medicoSelecionado,
          crm: med?.crm || '',
          valor_bruto_medico: n.bruto,
          retencao_individual: retencao,
          repasse: n.bruto * (1 - retencao/100)
        }] : []
        const totalRepasse = medicos_nota.reduce((a, m) => a + m.repasse, 0)
        const margem = recebido - totalRepasse

        const payload = {
          nf: n.nf, tomador: n.tomador, comp: n.comp, emissao: n.emissao,
          status: n.status, bruto: n.bruto, recebido, total_repasse: totalRepasse,
          margem, pct_margem: recebido > 0 ? margem/recebido : 0,
          medicos_nota: medicos_nota.length ? medicos_nota : null,
          nomes_medicos: medicoSelecionado || null,
          obs: n.discriminacao ? `Discriminação: ${n.discriminacao}` : null
        }

        const { data: nova } = await supabase.from('notas_fiscais').insert(payload).select().single()

        // Gerar comprovante se tiver médico
        if (medicoSelecionado && nova?.id) {
          await supabase.from('comprovantes').insert({
            token: uid(), nf_id: nova.id, medico_nome: medicoSelecionado,
            medico_crm: med?.crm || null, tomador: n.tomador,
            valor_repasse: n.bruto * (1 - retencao/100),
            competencia: n.comp, dados_extras: { nf: n.nf, pix: med?.chave_pix }
          })
        }
  ...
  const reiniciar = () => {
    setEtapa('upload'); setNotasImportadas([]); setErros([])
    setResultado(null); setSelecionadas(new Set()); setMedicoSelecionado(''); setRetencaoCustom('')
  }
  ...
  const toggleTodos = () => {
    if (selecionadas.size === notasImportadas.length) setSelecionadas(new Set())
    else setSelecionadas(new Set(notasImportadas.map((_, i) => i)))
  }
      ...
      {etapa === 'upload' && (
        <>
            ...
            <div className="card">
              ...
              <div className="card-body">
                <p style={{ fontSize: 13, color: 'var(--n4)', marginBottom: 14, lineHeight: 1.6 }}>
                  Compatível com o padrão <strong>ABRASF v2.04</strong> usado pelo Portal Nacional de NFS-e (gov.br) e prefeituras que adotam o padrão nacional.
                </p>
                <div
                  style={{ border: '2px dashed var(--border)', borderRadius: 'var(--radius-lg)', padding: 32, textAlign: 'center', cursor: 'pointer', transition: 'all .2s', background: 'var(--n10)' }}
                ...
                >
                  ...
                  <div style={{ marginTop: 12 }}>
                    <span style={{ background: 'var(--g10)', color: 'var(--g3)', border: '1px solid var(--g8)', borderRadius: 99, fontSize: 10, fontWeight: 700, padding: '2px 10px' }}>XML</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="card">
            ...
            <div className="card-body">
              <p style={{ fontSize: 13, color: 'var(--n4)', marginBottom: 12 }}>Use este modelo para criar sua planilha de importação:</p>
              ...
              <button className="btn btn-outline btn-sm" style={{ marginTop: 12 }} onClick={() => {
                const ws = XLSX.utils.aoa_to_sheet([
                  ['Número','Tomador','Valor','Competência','Emissão','Status'],
                  ['00001','Unimed Sergipe','1200,00','05/2026','15/05/2026','Emitida'],
                ])
        ...
        <>
            ...
            <div className="card" style={{ flex: 1, minWidth: 280 }}>
              ...
              <div className="card-body">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  ...
                  <div className="field">
                    <label>% Retenção (sobrescreve)</label>
          ...
          {erros.length > 0 && (
            <div style={{ background: 'var(--yellow-l)', border: '1px solid #FDE68A', borderRadius: 'var(--radius-lg)', padding: '12px 16px', marginBottom: 12, fontSize: 12, color: 'var(--yellow)' }}>
              ⚠️ {erros.length} aviso(s): {erros.slice(0,3).join(' | ')}{erros.length > 3 ? ` e mais ${erros.length-3}...` : ''}
            </div>
          )}
                ...
                <tbody>
                  {notasImportadas.map((n, i) => (
                    <tr key={i} style={{ opacity: selecionadas.has(i) ? 1 : .45 }}>
                      ...
                      <td className="mono" style={{ fontWeight: 700, color: 'var(--g3)' }}>
                        {Number(n.bruto).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                      </td>
      ...
      {etapa === 'resultado' && resultado && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 20 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--n1)', marginBottom: 8 }}>
